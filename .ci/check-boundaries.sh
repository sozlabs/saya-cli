...
if grep -nEi "${forbidden_deps_regex}" Cargo.toml >/dev/null; then
  echo "Found forbidden dependency token in Cargo.toml"
  grep -nEi "${forbidden_deps_regex}" Cargo.toml
  exit 1
fi
...
if grep -RInE --include='*.rs' "${forbidden_code_regex}" src >/dev/null; then
  echo "Found forbidden code pattern in src/"
  grep -RInE --include='*.rs' "${forbidden_code_regex}" src
  exit 1
fi

echo "[DoD0] Ensuring code references soz-saya endpoint semantics"
if ! grep -RInE --include='*.rs' 'soz-saya|/health|/messages|/stream' src >/dev/null; then
